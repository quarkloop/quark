...
// newQueryBuilder returns a query builder whose SQL is initialised with prefix.
// The strings.Builder must NOT be copied after being written to (its internal
// copyCheck guard panics — see https://pkg.go.dev/strings#Builder). We avoid
// the copy by allocating the queryBuilder struct directly and then writing
// into its embedded Builder.
func newQueryBuilder(prefix string) *queryBuilder {
        q := &queryBuilder{}
        q.sql.WriteString(prefix)
        return q
}
