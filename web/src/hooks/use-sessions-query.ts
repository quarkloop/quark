...
export function useSessions(agentId: string | undefined, baseUrl: string | undefined) {
  return useQuery<SessionRecord[]>({
    queryKey: agentId ? sessionsKey(agentId) : DISABLED_KEY,
    queryFn: () => get(`/api/v1/agents/${agentId}/sessions?${BASE(baseUrl!)}`),
    enabled: !!agentId && !!baseUrl,
  });
}

export function useCreateSession(agentId: string, baseUrl: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (req: CreateSessionRequest) =>
      post<CreateSessionResponse>(`/api/v1/agents/${agentId}/sessions?${BASE(baseUrl)}`, req),
    onSuccess(resp) {
...
export function useDeleteSession(agentId: string, baseUrl: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (sessionKey: string) =>
      del(`/api/v1/agents/${agentId}/sessions/${encodeURIComponent(sessionKey)}?${BASE(baseUrl)}`),
