...
```bash
make build-go      # builds quark-cli/quarkctl
```

The binary is at `quark-cli/quarkctl`. Copy it to your `$PATH` manually if you want
...
The `node` subcommand manages the node package registry (pushed `.ts`/`.jar`
payloads stored in the Catalog). Nodes live in `quark-nodes/` at the project root;
build compiles `src/` per `build.toml`, push packages the result into a zip
and sends it to the Catalog:

```bash
quarkctl node list                          # list all registered node packages
quarkctl node info quark/time/schedule/timer:v1   # show metadata for a package
quarkctl node search timer                  # search by keyword in URI/manifest
quarkctl node build quark/time/schedule/timer:v1  # compile src/ → target/*.jar (Java) or no-op (TS)
quarkctl node push  quark/time/schedule/timer:v1  # package + push to Catalog
quarkctl node pull  quark/time/schedule/timer:v1  # download a package's content
```
...
```bash
make build-go          # build quark-cli/quarkctl
