...
|-------|------|-------------|
| `listens` | `string[]` | Subjects the node subscribes to (relative, prefixed with `<system>.<namespace>.`) |
| `events` | `string[]` | Events the node may publish (becomes `<system>.<namespace>.<nodeName>.<event>`) |
...
NATS subjects encode the namespace: `monitor.alice.*` vs `monitor.bob.*`.
