...
func runStatsWatch() error {
	...
	for {
		ctx, cancel := context.WithTimeout(watchCtx, flagTimeout)
		namespaces, err := c.ListNamespaces(ctx)
		cancel()
		if err != nil {
			select {
...
func printStatsRow(c interface{ GetNamespace(context.Context, string) (*model.NamespaceDetail, error) }, namespace string) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	detail, err := c.GetNamespace(ctx, namespace)
	if err != nil {
		fmt.Fprintf(os.Stdout, "%-15s  error: %v\n", namespace, err)
		return
	}
