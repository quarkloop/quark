...
func (s *JSONLStore) Get(namespace, key string) ([]byte, error) {
	var e entry
	if err := s.col.GetInto(id(namespace, key), &e); err != nil {
...
func (s *JSONLStore) Set(namespace, key string, value []byte) error {
	return s.col.Set(entry{ID: id(namespace, key), Value: value})
}

// Delete removes the entry at namespace/key.
func (s *JSONLStore) Delete(namespace, key string) error {
	if err := s.col.Delete(id(namespace, key)); err != nil {
		if store.IsNotFound(err) {
...
// id combines namespace and key into a single store id.
func id(namespace, key string) string {
