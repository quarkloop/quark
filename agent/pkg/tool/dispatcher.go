...
// Invoker defines the interface for invoking tools.
...
// HTTPDispatcher invokes tools via their registered HTTP endpoints.
...
// Register adds a tool to the dispatcher. Safe for concurrent use.
...
// Invoke calls the named tool's HTTP endpoint with the given input.
...
func (d *HTTPDispatcher) Invoke(ctx context.Context, name string, input map[string]interface{}) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invoking tool %q: %w", name, err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("tool %q returned HTTP %d", name, resp.StatusCode)
	}
